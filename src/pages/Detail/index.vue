        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 以前的路由跳转是正常的从A路由跳转到B路由，这里在加入购车进行路由跳转之前，
    ...
    methods:{
      ...
      changeSkuNum(event){
        ...
        if(isNaN(value)||value<1){
          ...
          //如果用户输入了小数，取整parseInt，输入其他的就正常显示
      ...
      addShopcar(){
        ...
        /*
          ...
          下面这行代码说白了：调用仓库中的addOrUpdateShopCart,这个方法加上async，返回一定是个Promise，Promise有成功和失败的回调
        */
        try {
          ...
          //2.服务器存储成功--进行路由跳转
          //3.在路由跳转的时候还需要将产品额信息带给下一级的路由组件
          /* 一些简单的数据skuNum，通过query形式给路由组件传递过去
            产品信息的数据比较复杂（比如skuInfo是个对象），通过会话存储不持久化，会话结束数据会消失
            注意！！本地存储|会话存储，一般存储的是字符串（所以要把对象通过 JSON.stringify(this.skuInfo) 转化成字符串）
          */
