...
import QRCode from 'qrcode'
  export default {
    ...
    methods:{
      ...
      async open() {
        ...
        this.$alert(`<img src=${url} />`,"请你微信支付", {
          ...
          confirmButtonText:'已支付成功', //确认按钮的文本内容
          showClose:false, //右上角的叉子消失
          beforeClose:(type,instance,done)=>{
            ...
            if(type=='cancel'){
              ...
              //判断是否真的支付了(这里把if隐藏是为了每次跳转的时候不付钱)
              // if(this.code==200){
                clearInterval(this.timer)
                this.timer=null
                done()
    ...
    //工作的时候：尽量别在生命周期函数中async|await，可以在method中声明一个方法
    mounted(){
      this.getPayInfo();
    }
  }
