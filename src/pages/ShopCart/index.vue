  ...
  export default {
    ...
    methods:{
      ...
      handler:throttle(async function(type,disNum,cart){
          /*
        type:为了区分这三个元素
        disNum形参：+变化了(1) -变化量(-1) 当前变化量($event.target.value)
        cart:哪一个产品身上有id
        */
        switch(type){
          //加号
          case "add":
            ...
            break;
          case "minus":
            //判断产品的个数大于1，才可以传递给服务器-1,产品个数小于等于1，就传递0（原封不动）
            disNum = cart.skuNum>1?-1:0
            break;
          case "change":
            //用户输入进来的最终的量是一个非法的（带有汉字），带给服务器的数字还是0（不变）
      ...
      async updateChecked(cart,event){
        //带给服务器的参数isChecked，不应该是布尔值，应该是0|1
        try {
      ...
      async updateAllCartChecked(event){
        try{
          ...
          //派发一个action
          await this.$store.dispatch("updatedAllCartIsChecked",isChecked)
          this.getDate()
        }catch(err){
          alert(err.message)
        }
    ...
    computed:{
      ...
      //判断底部复选框是否勾选(全部商品选中才勾)
      isAllChecked(){
        //every全真才返回真==>只要全部元素的isChecked属性都为1才勾选
